...
class TrackingReportItem extends StatelessWidget {
  const TrackingReportItem({
    ...
    required this.reportId, required this.reportDescription,
  ...
  List<Color> _getStatusItemColor(String status){
    switch(status){
      ...
      case  'جاري تنفيذ البلاغ':
        return [Color(0xffD3BEA4),Color(0xffBA7929).withOpacity(.68),ColorManager.grey];
      case 'تم تنفيذ البلاغ':
        return [Color(0xffD3BEA4),Color(0xffBA7929).withOpacity(.68),Color(0xff3A8977).withOpacity(.64)];
      default:
